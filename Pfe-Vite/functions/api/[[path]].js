const slowAES = {
  aes: {
    ...
    core: function(i, t) {
      i=this.rotate(i);for(var r=0;r<4;++r)i[r]=this.sbox[i[r]];return i[0]=i[0]^this.Rcon[t],i;
    },
    ...
    mixColumn: function(i, t) {
      ...
      return i[0]=this.galois_multiplication(o[0],r[0])^this.galois_multiplication(o[3],r[1])^this.galois_multiplication(o[2],r[2])^this.galois_multiplication(o[1],r[3]),
             ...
             i[3]=this.galois_multiplication(o[3],r[0])^this.galois_multiplication(o[2],r[1])^this.galois_multiplication(o[1],r[2])^this.galois_multiplication(o[0],r[3]),i;
    },
    numberOfRounds: function(i) {
      switch(i){case this.keySize.SIZE_128:return 10;case this.keySize.SIZE_192:return 12;case this.keySize.SIZE_256:return 14;default:return null;}
    },
...
function toHex(d) {
  for (var e = "", f = 0; f < d.length; f++) e += (16 > d[f] ? "0" : "") + d[f].toString(16);
...
async function getInfinityFreeCookie() {
  ...
  try {
    const res = await fetch('https://morovista.xo.je/api/hotels', {
      headers: {
    ...
    if (match) {
      ...
      cachedCookie = toHex(decrypted);
      cookieTimestamp = Date.now();
      return cachedCookie;
    }
  } catch (e) {
    console.error('Error fetching cookie challenge', e);
  }
  return cachedCookie || '';
}
...
export async function onRequest(context) {
  ...
  const targetUrl = 'https://morovista.xo.je' + url.pathname + url.search;
  const cookie = await getInfinityFreeCookie();

  const newHeaders = new Headers(request.headers);
  if (cookie) {
    newHeaders.set('Cookie', `__test=${cookie}`);
  }
  ...
  try {
    ...
    if (contentType.includes('text/html')) {
      ...
      const match = text.match(/var a=toNumbers\("([a-f0-9]+)"\),b=toNumbers\("([a-f0-9]+)"\),c=toNumbers\("([a-f0-9]+)"\);/);
      if (match) {
        const a = toNumbers(match[1]);
        const b = toNumbers(match[2]);
        const c = toNumbers(match[3]);
        cachedCookie = toHex(slowAES.decrypt(c, 2, a, b));
        cookieTimestamp = Date.now();
        newHeaders.set('Cookie', `__test=${cachedCookie}`);
        const retryRes = await fetch(targetUrl, { ...init, headers: newHeaders });
